...
public class ByteBlob implements Blob {
	...
	public static final class Diff implements Blob.Diff {
		...
		private final ByteBlob.Replacement[] replacements;
		 ...
		 * @param deltas
		 */
		public Diff(Blob parent, ByteBlob.Replacement... deltas) {
			if (deltas == null) {
		...
		public int size() {
			...
			for (int i = 0; i != replacements.length; ++i) {
				ByteBlob.Replacement ith = replacements[i];
		...
		@Override
		public byte readByte(int index) {
			int i = ByteBlob.findEnclosing(replacements, index);
			//
			if (i >= 0) {
				...
				int delta = ByteBlob.delta(0, -i - 1, replacements);
				// Read from parent.
		...
		public byte[] readAll() {
			...
			for (int i = 0; i < replacements.length; ++i) {
				final ByteBlob.Replacement ith = replacements[i];
		...
		@Override
		public Diff writeByte(int index, byte b) {
			ByteBlob.Replacement[] rs = write(replacements, index, 1, b);
			return new ByteBlob.Diff(parent, rs);
		}
		...
		public Diff writeShort(int offset, short value) {
			...
			byte b1 = (byte) ((value >> 8) & 0xFF);
			byte b2 = (byte) (value & 0xFF);
			ByteBlob.Replacement[] rs = write(replacements, offset, 2, b1, b2);
			return new ByteBlob.Diff(parent, rs);
		}
		...
		public Diff writeInt(int offset, int value) {
			...
			byte b3 = (byte) ((value >> 8) & 0xFF);
			byte b4 = (byte) (value & 0xFF);
			ByteBlob.Replacement[] rs = write(replacements, offset, 4, b1, b2, b3, b4);
			return new ByteBlob.Diff(parent, rs);
		}

		@Override
		public Diff writeBytes(int offset, byte... bytes) {
			ByteBlob.Replacement[] rs = write(replacements, offset, bytes.length, bytes);
			return new ByteBlob.Diff(parent, rs);
		}

		@Override
		public Diff replaceBytes(int offset, int length, byte... bytes) {
			ByteBlob.Replacement[] rs = write(replacements, offset, length, bytes);
			return new ByteBlob.Diff(parent, rs);
		}

		@Override
		public Diff insertByte(int offset, byte b) {
			ByteBlob.Replacement[] rs = write(replacements, offset, 0, b);
			return new ByteBlob.Diff(parent, rs);
		}
		...
		public Diff insertShort(int offset, short value) {
			...
			byte b1 = (byte) ((value >> 8) & 0xFF);
			byte b2 = (byte) (value & 0xFF);
			ByteBlob.Replacement[] rs = write(replacements, offset, 0, b1, b2);
			return new ByteBlob.Diff(parent, rs);
		}
		...
		public Diff insertInt(int offset, int value) {
			...
			byte b3 = (byte) ((value >> 8) & 0xFF);
			byte b4 = (byte) (value & 0xFF);
			ByteBlob.Replacement[] rs = write(replacements, offset, 0, b1, b2, b3, b4);
			return new ByteBlob.Diff(parent, rs);
		}

		@Override
		public Diff insertBytes(int offset, byte... bytes) {
			ByteBlob.Replacement[] rs = write(replacements, offset, 0, bytes);
			return new ByteBlob.Diff(parent, rs);
		}
		...
		public Blob merge(Blob blob) {
			if (blob == this || parent == blob) {
				return this;
			} else if (blob instanceof ByteBlob.Diff) {
				ByteBlob.Diff d = (ByteBlob.Diff) blob;
				if (d.parent() == parent) {
					ByteBlob.Replacement[] rs = ByteBlob.merge(replacements, d.replacements);
					return new ByteBlob.Diff(parent, rs);
				}
			}
			throw new IllegalArgumentException("cannot merge blob");
	 ...
	 *
	 */
	public static final class Replacement implements Blob.Replacement, Comparable<ByteBlob.Replacement> {
		/**
		 ...
		 * @return
		 */
		public boolean disjoint(ByteBlob.Replacement other) {
		...
		@Override
		public int compareTo(ByteBlob.Replacement o) {
			if (offset < o.offset) {
				return -1;
	 ...
	 * @return
	 */
	private static boolean areDisjoint(ByteBlob.Replacement[] deltas) {
		for (int i = 1; i < deltas.length; ++i) {
			ByteBlob.Replacement ithm1 = deltas[i - 1];
			ByteBlob.Replacement ith = deltas[i];
	 ...
	 * @return
	 */
	private static ByteBlob.Replacement[] merge(ByteBlob.Replacement[] lhs, ByteBlob.Replacement[] rhs) {
		...
		// end.
		ByteBlob.Replacement[] results = new ByteBlob.Replacement[lhs.length + rhs.length];
		//
		int l = 0, r = 0, k = 0;
		//
		while(l < lhs.length && r < rhs.length) {
			ByteBlob.Replacement lth = lhs[l];
			ByteBlob.Replacement rth = rhs[r];
			//
			...
			if(lth_last <= rth_offset) {
				if(lth_last == rth_offset) {
					...
					results[k++] = new ByteBlob.Replacement(lth_offset,lth.length + rth.length,bytes);
					l = l + 1;
					r = r + 1;
				} else {
					// lth first
				...
				if(rth_last == lth_offset) {
					...
					results[k++] = new ByteBlob.Replacement(rth_offset,lth.length + rth.length,bytes);
					l = l + 1;
					r = r + 1;
				} else {
					// rth first
	 ...
	 * @return
	 */
	private static int findLowestAffected(ByteBlob.Replacement[] replacements, int offset) {
		int i = 0;
	 ...
	 * @return
	 */
	public static int findEnclosing(ByteBlob.Replacement[] replacements, int offset) {
		int i = 0;
		int j = replacements.length - 1;
		//
		while (i <= j) {
	 ...
	 * @return
	 */
	private static ByteBlob.Replacement[] write(int index, ByteBlob.Replacement[] replacements, Replacement r) {
	 ...
	 * @return
	 */
	private static ByteBlob.Replacement[] write(int i, int j, ByteBlob.Replacement[] replacements, ByteBlob.Replacement r) {
	...
	public static void main(String[] args) {
		...
		Blob b2 = b1.writeBytes(1, (byte) 'E', (byte) '?');
		Blob b3 = b2.writeBytes(0, (byte) 'H',(byte) '_',(byte) '_',(byte) '_');
		//
